  ...
  useEffect(() => {
    ...
    const initCheckout = async () => {
      try {
        ...
        const message =
          err instanceof Error ? err.message : "Không th�?khởi tạo thanh toán";
        setError(message);
      } finally {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-surface-page p-6 text-center">
        <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-[2.5rem] shadow-2xl shadow-slate-200/50 border border-slate-50">
          ...
          <Link href={`/webview/order/${orderId}`} className="flex items-center justify-center gap-2 text-xs font-black text-slate-400 hover:text-accent transition-colors uppercase tracking-widest group">
      ...
      <div className="relative z-10 max-w-sm w-full p-6 text-center space-y-8">
        ...
        <div className="space-y-3">
          ...
          <p className="text-sm font-medium text-slate-500 leading-relaxed px-4">
            Đang kết nối an toàn với h�?thống thanh toán Toss... Vui lòng không đóng trình duyệt.
          </p>
        </div>
