...
export default function InventoryPage() {
  ...
  const stats = useMemo(() => {
    return {
      ...
      totalValue: products.reduce((acc: number, p: any) => acc + (p.price || 0) * (p.quantity || 0), 0),
    };
  }, [products]);
      ...
      <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-4xl font-black tracking-tight text-slate-900 uppercase">Kho hàng</h1>
          </div>
          <p className="text-sm font-semibold text-slate-400">Quản lý sản phẩm, tồn kho và đồng bộ dữ liệu đa kênh.</p>
        </div>
        <div className="flex flex-wrap gap-4">
          <button 
            onClick={() => setShowImportWizard(true)}
            className="group flex items-center gap-3 rounded-2xl bg-white px-6 py-4 text-sm font-black text-slate-900 shadow-xl shadow-slate-200/50 ring-1 ring-slate-200 transition-all hover:bg-slate-50 active:scale-95"
          >
          ...
          <button 
            onClick={openCreate}
            className="ai-glow flex items-center gap-3 rounded-2xl bg-slate-900 px-8 py-4 text-sm font-black text-white shadow-2xl transition-all hover:bg-slate-800 hover:-translate-y-1 active:scale-95"
          >
