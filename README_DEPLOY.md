...
**Nếu sau đăng nhập vẫn về dashboard cũ** (ví dụ `https://pure-curiosity-production-f937.up.railway.app`): đó là **Supabase URL Configuration** hoặc **bản deploy Railway** vẫn là app cũ — không phải do “tạo thêm một giao diện mới” trong repo.

1. **Supabase** → **Authentication** → **URL Configuration**: đặt **Site URL** = URL dashboard **đang dùng thật** (domain service Next.js mới hoặc Vercel). Trong **Redirect URLs** thêm `https://<domain-mới>/auth/callback` và các URL localhost/HTTPS dev bạn dùng.
2. **Email templates** (magic link): dùng biến `{{ .SiteURL }}` để link trong mail khớp Site URL mới.
3. **Railway / hosting:** thay thế UI cũ bằng cách **deploy lại** service dashboard từ repo này (hoặc đổi branch/image) — nếu production vẫn chạy service `pure-curiosity-...`, người dùng vẫn thấy giao diện cũ dù code local đã đổi.
4. (Tùy chọn) Trên frontend: `NEXT_PUBLIC_AUTH_REDIRECT_ORIGIN=https://<dashboard-mới>` — ép `redirectTo` OAuth/email về đúng origin khi cần (đồng thời vẫn phải khai báo URL đó trong Supabase Redirect URLs).
...
**Google Sheets + Google Picker (chọn file từ Drive):** Backend cần đủ `GOOGLE_OAUTH_CLIENT_ID`, `GOOGLE_OAUTH_CLIENT_SECRET`, `GOOGLE_OAUTH_REDIRECT_URI` và **`GOOGLE_PICKER_API_KEY`** (Google Cloud Console: bật **Google Picker API** + giới hạn HTTP referrer đúng origin Dashboard). OAuth scope hiện tại gồm `spreadsheets.readonly` + `drive.metadata.readonly`; nếu đổi scope, người dùng cần **kết nối lại** Google trong app.
