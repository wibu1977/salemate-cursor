...
async def _exchange_long_lived_page_token(short_token: str) -> str:
    ...
    try:
        ...
        logger.warning("Token exchange request failed: %s. Using original token.", str(e))
        return short_token
...
async def connect_page(
    ...
    try:
        ...
        try:
            ...
            logger.error("Meta API error during subscription: %s", str(e), exc_info=True)
            raise HTTPException(
                status_code=502,
                detail=f"Meta API error: {str(e)}",
            )

        # ── 3. Cài đặt Messenger UI (Non-critical, don't fail if these fail) ──────
        try:
            ...
            logger.warning("Failed to setup Messenger UI (non-critical): %s", str(e))
        ...
        try:
            ...
            if page:
                ...
                page.page_access_token = long_lived_token
                page.platform = payload.platform
                page.is_active = True
                logger.info("Updated existing page %s", payload.page_id)
            else:
                page = ShopPage(
            ...
            logger.info(
                "Connected page %s (%s) to workspace %s",
                payload.page_name,
                payload.page_id,
                workspace_id,
            )
            ...
            await db.rollback()
            logger.error("Database error while connecting page: %s", str(e), exc_info=True)
            raise HTTPException(
                status_code=500,
        ...
        logger.error("Unexpected error in connect_page: %s", str(e), exc_info=True)
        raise HTTPException(
            status_code=500,
